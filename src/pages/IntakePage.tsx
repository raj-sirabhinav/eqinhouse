...
export const IntakePage: React.FC = () => {
  ...
  // Parse ACV range to clean integer value
  const parseAcvToNumber = (val: string): number => {
    let acvValue = 10000;
    if (val.includes('$15k–$25k') || val.includes('15k-25k')) acvValue = 20000;
    else if (val.includes('$25k–$50k') || val.includes('25k-50k')) acvValue = 35000;
    else if (val.includes('$50k–$100k+') || val.includes('50k-100k')) acvValue = 75000;
    return acvValue;
  };

  // Derived domain and company name client-side directly from work_email
  ...
  const rootPart = domain ? domain.split('.')[0] : '';
  const companyNameUpper = rootPart ? rootPart.toUpperCase() : '';
  const companyNameDisplay = rootPart
    ? rootPart
        .split(/[-_]/)
        .filter(Boolean)
  ...
  // Helper to compile unified payload adhering strictly to the user schema
  const getFormDataPayload = () => {
    ...
    const acvValue = parseAcvToNumber(acvRange);

    return {
      full_name: fullName.trim(),
      work_email: cleanEmail,
      website_domain: domain,
      company_name: companyNameUpper,
      company_arr_tier: arrRange,
      average_contract_value_acv: acvValue,
          ...
          <AnimatePresence mode="wait">
            {!isSubmitted ? (
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* =============================================================
                    STEP 1: COMPANY PROFILE (CLEAN LIGHT-MODE LAYOUT)
                   ============================================================= */}
                {currentStep === 1 && (
                  <motion.div
                    key="step1"
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        />
                        <User className="absolute right-3.5 top-3 h-4 w-4 text-[#a89f91]" />
                      </div>
                      {fullNameError ? (
                        ...
                        <p className="mt-1 text-xs text-[var(--text-secondary)]">
                          Your dedicated lead architect point of contact.
                        </p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {[
                          { label: '<$15k (Low ROI for Custom)', val: '<$15k (Low ROI for Custom)' },
                          ...
                          <button
                            ...
                            className={`py-2.5 px-3 rounded-lg text-xs font-medium border text-left transition-all cursor-pointer ${
                              acvRange === item.val
                                ? 'bg-[#6366F1] text-white border-[#6366F1] shadow-xs font-semibold'
                                : 'bg-white border-[var(--border-subtle)] text-[var(--text-primary)] hover:bg-[#faf7f3]'
                            }`}
                    ...
                    <div className="pt-3 flex justify-end">
                      <button
                        ...
                        className="inline-flex items-center px-5 py-2.5 rounded-lg text-xs font-medium text-white bg-[#6366F1] hover:bg-[#4f46e5] transition-colors shadow-xs cursor-pointer"
                      >
                  ...
                  >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-3.5 py-2.5 rounded-lg border border-[var(--border-subtle)] text-[var(--text-primary)] text-xs font-medium focus:outline-none focus:border-[#6366F1] bg-white cursor-pointer"
                      >
                    ...
                    <div className="pt-3 flex justify-between">
                      <button
                        ...
                        className="inline-flex items-center px-4 py-2 rounded-lg text-xs font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)] border border-[var(--border-subtle)] bg-white cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="inline-flex items-center px-5 py-2.5 rounded-lg text-xs font-medium text-white bg-[#6366F1] hover:bg-[#4f46e5] transition-colors shadow-xs disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer"
                      >
                        {isSubmitting ? (
                          <>
                ...
                {/* =============================================================
                    STEP 3: CONDITIONAL CONFIRMATION VIEW (GATED BY ACV >= $25k)
                   ============================================================= */}
                {currentStep === 3 && (
                  <motion.div
                    key="step3"
                  ...
                  >
                    {parseAcvToNumber(acvRange) >= 25000 ? (
                      /* -------------------------------------------------------------
                      ...
                      <>
                        <div className="space-y-2">
                          ...
                          <p className="text-xs sm:text-sm text-[#334155] leading-relaxed pt-1 font-sans">
                            Choose a time with our systems team below. Pre-filled with your intake parameters.
                          </p>
                        </div>
                        ...
                        <div className="p-5 rounded-xl bg-white border border-[#E2E8F0] shadow-xs space-y-3 font-sans">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
                            ...
                            <div className="p-3 rounded-lg bg-[#FAF8F5] border border-[#E2E8F0]/80">
                              ...
                              <div className="text-xs sm:text-sm font-semibold text-[#0F172A] font-mono truncate" title={fullName || email}>
                        ...
                        <div className="rounded-xl border border-[#E2E8F0] bg-white overflow-hidden shadow-xs p-5 sm:p-6 font-sans space-y-4">
                          ...
                          <div className="w-full min-h-[650px] rounded-xl bg-[#0F172A] border border-[#1E293B] shadow-xl overflow-hidden p-1 sm:p-2">
                            <Cal
                              ...
                              config={{
                                name: fullName || companyNameDisplay || 'Executive Lead',
                                email: cleanEmail,
                                theme: 'dark',
                                layout: 'month_view',
                              }}
                      ...
                      <div className="space-y-6">
                        {/* Success Header Card */}
                        <div className="rounded-xl border border-[#E2E8F0] bg-white p-6 sm:p-8 shadow-xs text-center space-y-4 font-sans">
                          {/* Green Checkmark Badge */}
                          <div className="h-12 w-12 mx-auto rounded-full bg-emerald-50 border border-emerald-200 flex items-center justify-center text-emerald-600 shadow-xs">
                            <CheckCircle2 className="h-6 w-6" />
                          </div>

                          <div className="space-y-1.5">
                            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-[11px] font-semibold font-mono uppercase tracking-wider">
                              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
                              <span>SUBMISSION CONFIRMED</span>
                            </div>
                            <h2 className="font-serif text-2xl sm:text-3xl text-[var(--text-primary)] font-normal tracking-tight">
                              Intake Parameters Received
                            </h2>
                          </div>

                          <p className="text-sm sm:text-base text-[var(--text-primary)] max-w-lg mx-auto leading-relaxed">
                            Thank you {fullName ? <span className="font-semibold">{fullName}</span> : ''} for submitting your architecture profile for{' '}
                            <span className="font-semibold text-[#0F172A]">{companyNameDisplay || 'your company'}</span>. We’ve sent our complete Inbound Speed-to-Lead &amp; Data Latency Diagnostic Framework directly to{' '}
                            <span className="font-semibold text-[#0F172A] font-mono text-xs sm:text-sm">{cleanEmail || email}</span>.
                          </p>

                          <div className="p-4 rounded-lg bg-[#FAF8F5] border border-[#E2E8F0] text-xs text-[var(--text-secondary)] text-left leading-relaxed">
                            <span className="font-semibold text-[var(--text-primary)] block mb-0.5 font-mono text-[11px] uppercase tracking-wider">
                              Next Step Review Protocol
                            </span>
                            Our engineering team reviews submissions against our pipeline benchmarks. If you have questions about your current CRM plumbing, feel free to reply directly to that email.
                          </div>
                          ...
                          <div className="pt-2 flex justify-center">
                            <Link
                              to="/"
                              className="inline-flex items-center justify-center px-6 py-2.5 rounded-lg text-xs sm:text-sm font-semibold text-white bg-[#181a20] hover:bg-[#2c2f38] transition-all shadow-xs"
                            >
                              <ArrowLeft className="mr-2 h-4 w-4" />
                        ...
                        <div className="p-5 rounded-xl bg-white border border-[#E2E8F0] shadow-xs space-y-3 font-sans">
                          <div className="flex items-center justify-between pb-2.5 border-b border-[#F1F5F9] text-[11px] font-mono">
                            ...
                            <span className="px-2 py-0.5 rounded bg-emerald-50 text-[10px] text-emerald-700 font-medium border border-emerald-200">
                              SYNCHRONIZED
                            </span>
                          </div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
                            <div className="p-3 rounded-lg bg-[#FAF8F5] border border-[#E2E8F0]/80">
                              ...
                              <div className="text-xs sm:text-sm font-semibold text-[#0F172A] font-mono truncate">
                                {companyNameDisplay || domain || 'Direct Intake'}
                              </div>
                            </div>
                            <div className="p-3 rounded-lg bg-[#FAF8F5] border border-[#E2E8F0]/80">
                              <div className="text-[11px] font-mono text-[var(--text-secondary)] uppercase tracking-wider mb-1">
              ...
              >
                ...
                <div className="rounded-xl border border-[#E2E8F0] bg-white p-5 space-y-3 text-xs font-sans">
                  <div className="flex items-center justify-between pb-2 border-b border-[#F1F5F9]">
                    ...
                    <span className="font-semibold text-[#0F172A] font-mono">{fullName ? `${fullName} (${email})` : email}</span>
                  </div>
                  <div className="flex items-center justify-between pb-2 border-b border-[#F1F5F9]">
